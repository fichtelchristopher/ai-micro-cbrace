...
def get_pretrained_classifier(classifier_dir, classifier_name, classifier_type, output_dir = ""):
    '''
    ...
    if classifier_type in SHALLOW_IMPLEMENTATIONS:
        classifier_paths = glob.glob(classifier_dir + f"/{classifier_name}.pkl", recursive=False)
        assert(os.path.exists(classifier_paths[0]))
        classifier = load_from_pickle(classifier_paths[0])
    else:
    ...
    if len(output_dir) > 0:
        classifier.set_classifier_output_dir(output_dir)

    return classifier

def main(classifier: Classifier, classifier_type, test_database, test_dataset, run_transition_analysis = True, label_mapping = None):
    '''
    The main method.
    '''
    ...
    if not run_transition_analysis:
    ...
    for test_file in test_files:
        ...
        from_code = -1
        to_code = 200
        test_file_name = os.path.basename(test_file).replace(".pkl", "")
        transition_output_dir = classifier.evaluation_output_dir + f"/Transitions/{test_file_name}"

        transition_stat_df = transition_analysis(probabilities, predictions, gt_df, data_df, classifier.activity_code_type_dict , from_code = from_code, to_code = to_code, transition_visualisation_range = 200, output_dir = transition_output_dir, visualise_transitions=True)  
...
if __name__ == "__main__":
    ...
    # Set the test database _____________________________________________________________________________________________________________
    ...
    classifier_base_dir = f"{AI_PIPELINE_DIR}/{train_database}/DS{train_dataset}/05-Classifier" # this will also be used for saving the statistics file
    ...
    if os.path.exists(stat_output_f):
        ...
        stat_df = pd.DataFrame(columns = ["classifier_dir", "classifier_name", "test_db", "test_dataset", "test_acc", "test_f1", "test_ce_loss"])

    step_sizes = [250] #, 500, 1000, 2500]

    # Run the loop 
    for classifier_type in ["CNN_LSTM_SIMPLE"]:# NN_IMPLEMENTATIONS + SHALLOW_IMPLEMENTATIONS:  #"CNN_LSTM_SIMPLE"]: #[NN_IMPLEMENTATIONS + SHALLOW_IMPLEMENTATIONS]
        default_classifier_dir = f"{classifier_base_dir}/{classifier_type}"
        classifier_default_name = f"{train_database}_{classifier_type}_on_{test_database}"
        for step in step_sizes: #step_sizes:
            for subdir in os.listdir(default_classifier_dir):
                if classifier_type in NN_IMPLEMENTATIONS:
                    if (f"s{step}_" not in subdir):
                        continue
                else:
                    if (f"s_{step}_" not in subdir):
                        continue
                ...
                classifier.pipeline_configuration["STEP_SIZE_TEST"] = 1
                if classifier_type in NN_IMPLEMENTATIONS:
                    classifier.pipeline_configuration["TRAIN_PER_FILE"] = False
                classifier.classifier_name = classifier_default_name

                if classifier.pipeline_configuration["SAMPLING_MODE"] == "fixed":
                    continue

                test_acc, test_f1, test_ce_loss = main(classifier, classifier_type, test_database, test_dataset, run_transition_analysis = True, label_mapping = None)  

                results_dict = {
                    "classifier_dir": classifier.classifier_output_dir,
